...
public class ShortestPathWithAlternatingColors {
    ...
    public class Solution {
        ...
        public int[] shortestAlternatingPaths(int n, int[][] redEdges, int[][] blueEdges) {
            ...
            int[] redStep = new int[n];
            int[] blueStep = new int[n];

            for (int i = 1; i < n; i++) {
                redStep[i] = Integer.MAX_VALUE;
                blueStep[i] = Integer.MAX_VALUE;
            }

            Queue<int[]> queue = new ArrayDeque<>();
            queue.add(new int[]{0, 0}); //0代表下一步是红色
            queue.add(new int[]{0, 1}); //1代表下一步是蓝色
            ...
            for (int i = 0; i < n; i++) {
                if (blueStep[i] < redStep[i]){
                    redStep[i] = blueStep[i];
                }
                if (redStep[i] == Integer.MAX_VALUE){
                    redStep[i] = -1;
                }
            }
