...
public class DesignAuthenticationManager {
    ...
    class AuthenticationManager {
        int timeToLive;
        Map<String, Integer> tokenId2OutTime = new HashMap<>();
        ...
        public void generate(String tokenId, int currentTime) {
            tokenId2OutTime.put(tokenId, currentTime + timeToLive);
        }

        public void renew(String tokenId, int currentTime) {
            //包含
            if (tokenId2OutTime.containsKey(tokenId)) {
                Integer integer = tokenId2OutTime.get(tokenId);
                //未过期
        ...
        public int countUnexpiredTokens(int currentTime) {
            List<Integer> collect = tokenId2OutTime.values().stream().filter(e -> e > currentTime).collect(Collectors.toList());
            return collect.size();
        }
    }
